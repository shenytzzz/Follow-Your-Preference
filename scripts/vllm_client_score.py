"""Bulk scoring client for UnifiedReward (processes all items, no early exit)."""
...
class VLMessageClient:
    """Helper to encode images & build multimodal chat prompts."""
...
def gen_results(
    ...
    # print(f"Total {len(messages)} messages to process.")

    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(task, item) for item in messages]
        # print(f"Total {len(futures)} futures submitted.")

        with open(output_path, "a") as fout:
            # print(f"Total {len(futures)} futures to complete.")
