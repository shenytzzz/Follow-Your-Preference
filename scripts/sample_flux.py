#!/usr/bin/env python
# coding=utf-8
"""Sample FLUX Fill outputs to create annotation JSON for DPO / evaluation."""
...
class MyDataset:
    """Collate: decode mask, resize preserving aspect ratio, random crop."""
    ...
    def __call__(self, examples):
        ...
        for example in examples:
            ...
            urls.append((image_id.split("_")[0], image_id.split("_")[1]))
            ...
            mask_strs.append(segmentation["mask"][choice])
...
def main():
    """Main entry: generate images and write per-process result JSON."""
    ...
    if args.seed is not None:
        logger.info(f"[{accelerator.process_index}] Setting seed to {args.seed}")
        set_seed(args.seed)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        # init_images, mask_images, captions, urls, crops, types, mask_choices = batch["pixel_values"], batch["masks"], batch["captions"], batch["urls"], batch["crops"], batch["types"], batch["mask_choices"]
        (
            init_images,
            mask_images,
            captions,
        ...
        def save_and_check(_image):
            i, image = _image
            if not os.path.exists(Path(img_output_dir, urls[i][0])):
                os.makedirs(Path(img_output_dir, urls[i][0]), exist_ok=True)
            if check_first:
                logger.info(
                    f"[{accelerator.process_index}] check first for {urls[i][0].split('/')[-1].split('.')[0]}_{urls[i][1]}"
                )
                if os.path.exists(
                    f"{Path(Path(args.output_dir.replace("batch_2", "batch_1"), "images"), urls[i][0], f'{urls[i][1]}.png')}"
                ):
                    raise ValueError(
                        f"[{accelerator.process_index}] {Path(img_output_dir, urls[i][0].split('/')[-1].split('.')[0], f'{urls[i][1]}.png')} already exists"
                    )
            image.save(f"{Path(img_output_dir, urls[i][0], f'{urls[i][1]}.png')}")
            result[f"{urls[i][0]}_{urls[i][1]}"] = {
        ...
        _ = list(map(save_and_check, enumerate(images)))
